import collections
from models.deeptravel.DeepTravel_Preprocessing import helpers
import argparse
from models.deeptravel.DeepTravel_Preprocessing import logger
import numpy as np
import os
...
def main():
    ...
    os.makedirs(args.ttf_destination_folder, exist_ok=True)
    os.makedirs(args.data_destination_folder, exist_ok=True)

    # initialize arrays for short-term and long-term traffic features
    speed_array = 'speeds'
    time_array = 'times'
    short_ttf = [
        [collections.defaultdict(lambda: {speed_array: [], time_array: []}) for _ in range(128)] for _ in range(128)
    ]
    long_ttf = [
        [collections.defaultdict(lambda: {speed_array: [], time_array: []}) for _ in range(128)] for _ in range(128)
    ]
